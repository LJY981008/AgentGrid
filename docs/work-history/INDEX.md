...
| 2026-06-24 | A-1 데이터 정제(S6-b G-7 블로커) — EODHD **영속 소스결함** 2종 규명(디버깅 규율·라이브 재요청 동일): ①0-OHLCV 비거래일봉 506,556(close<=0·drop) ②open=전일종가 carry-forward·close>high 등 OHLC위반 123,309(close anchor 보정). `normalize_ohlc`(공유·verify동형·멱등)+ingest guard+`clean_ohlc` migration(15,107파일 타겟). 리뷰 CRITICAL(셀렉터 nonpositive 누락) 반영. **verify(G-7) PASS**(price<=0=0·ohlc=0·99.6M행). 다음=A-2 OOM | 일반 구현 | [[2026-06-24-A1-데이터정제-0price-OHLC]] |
| 2026-06-24 | A1p2 상한 garbage 정제(G-3 폭발 해소) — EODHD가 상장이전/결측을 **$1M max-value sentinel로 back-pad**(LUV/MRVL legit 침투)→등가중 벤치 복리 e80 발산(G-3 oos_excess −10^80=수치버그). `is_sentinel_bar` per-bar L1(raw $1M±band)·L2(adjusted $1M pin·HALF_UP)·L3(adj_factor≥100,000)+ingest/clean/verify(G-7 sentinel_count=0) 단일 SQL 출처. **L4 수익률 상한 cap**(engine+bench `min(ret,+19.0)`·GME 16.2x 보존). 리뷰 반영: floor 제거(ret≥−1 수학경계→낙관편향 0)·cap을 rule_signature 포함(동결 우회 봉인)·폐지 −100% 보존. **A1p2-6 실행**: clean→verify(G-7) PASS(98.24M행·sentinel 0)→cache→게이트 완주. **판정 FAILED(validated=false 유지)** — worst_oos_excess −47(직전 −10^80·e80 폭발 72자릿수 제거 ✅). 단 적대검증: FAIL=momentum 무알파 아니라 **측정척도 오염**(decay>1 7/10·비용불변·잔존 microcap penny+cap복리 e7 폭발)→robustness 미측정. 다음=**PIT 유동성/가격 하한 필터**(별도) | 일반 구현 | [[2026-06-24-A1p2-상한garbage-정제]] |
| 2026-06-25 | 백테스트 방법론 전면 교정 — momentum 공정 측정→validated 판정. 다전문가(미국주식 퀀트·구현·적대비판) 토의·2회 리뷰: 2차 폭발(oos_excess e7) 근본=microcap 아니라 **측정 셋업**(top-5×+19cap×산술복리×등가중-전체 벤치). 유동성필터-only=band-aid → **방법론 전면 교정**(수익처리 root fix[+19cap→±100% clip→로그]·top decile·동일비용 벤치·유니버스 PIT 필터·신뢰구간 동결). Phase 0 진단(pret분포·시총가용·데이터신뢰구간)→Phase 1 ADR-010 일괄동결(R1 방화벽·R2 측정게이트)→게이트 1회→판정. 양 전문가 ACCEPT-WITH-RESERVATIONS(R1~R4·0d 반영) | 플랜모드 승인 | [[2026-06-25-백테스트방법론-전면교정]] |
| 2026-06-29 | 다팩터 피벗 Sub-project A — 재무 데이터 PIT 파운데이션. 공식 게이트 validated=false(momentum 무알파 확정) → 다팩터 결합 정당. **병목=재무 데이터**(probe 실측 커버 0.03%·ROE 7~8종목). A1 폐지 ticker→cik 복구(EODHD ID-Mapping/SEC cik-lookup)·A2 PIT ticker_history+PitIdentityResolver(시점 무시 버그 fix·다중매칭 raise·DI 0변경)·A3 companyfacts 백필(증분resume·2009+·Parquet)·A4 커버리지 probe. 브레인스토밍→다관점 워크플로우 9에이전트+tech-researcher+3Explore+2Plan | 플랜모드 승인 | [[2026-06-29-다팩터-재무데이터-PIT파운데이션]] |
