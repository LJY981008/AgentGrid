...
**다음 단계**: **S6 데이터 신뢰성 게이트 + 백테스트 실검증**. 백테스트 엔진 골격은 완료(`backtest/` 14모듈·rolling as_of·forward-return·폐지청산·CAGR/Sharpe/MDD·IS/OOS 워크포워드·purge·decay·등가중 벤치). 선결: ①EODHD 결제($19.99/월) 다년 history ②S5 전체 유니버스(폐지 포함) 적재 + 종목마스터(listed/delisted) → 엔진 `UniversePort` 실데이터 교체(현 골격은 가격기반 FakeUniversePort) ③EDGAR ticker→cik 매핑(IdentityResolver 실전) ④S6 게이트 통과. **그 전까지 백테스트 수치 신뢰·`meta.validated=true` 금지**(§4.1 BLOCKING). 병행 후속: webapp BacktestPage placeholder 교체(+/api/backtest·Recharts).
