...
2. **🔴 신뢰구간 게이트 1회(미실행·메모리 근본 블로커)**: `canonical_gate_config()`(decile·2000~2026)·walk_forward(IS+OOS×3cost×N fold). ⚠️ **단일 decile run 이 전 유동 유니버스서 9-12GB peak = 12g cgroup 근본 초과**(비결정적 OOM·실측 light2/3/4 전부 OOM). **MEM-fix 완료**(커밋 6638505·between-run reset=cross-run 누적 차단·gate2 11.9→4.4GB 실측)나 **단일 run peak 는 못 낮춤**(within-run reconnect=spike 해로움·malloc_trim=DuckDB 내부 버퍼에 무효·둘 다 폐기). → **full 게이트 ≥20g 박스 필요**(이전 게이트도 20g). 현 호스트 16GB 여유(타컨테이너 14GB) 부족 → **사용자가 메모리 충분 환경서 실행**. R2/G-1~8→validated. pre-registration: 게이트 1회.
   - 격리 실행 예(메모리 확보·app·web 정지 후): `docker run --memory 20g -v stockpick_parquet-data:/app/data/parquet -e STOCKPICK_DATA_DIR=/app/data/parquet -e STOCKPICK_DUCKDB_MEMORY_LIMIT=4GB -e MALLOC_ARENA_MAX=2 <image> python -m stockpick.backtest.s6_gate`. 단계 확인=sanity 패턴(`scratchpad/sanity_decile_gate.py`·`oom={{.State.OOMKilled}}`). ⚠️ `docker rm/kill` pre-bash-guard 차단(orphan: sanity2/light{,2,3,4}/gate{1,2}/diag{,2}·`*_out.txt` temp=사용자 정리).
   - ✅ **축소 walk_forward 게이트 완주**(gate4·14g·`--memory-swap 14g`=freeze 차단·MEM-fix between-run reset 가 메모리 2~12GB 진동 바운드·oom=false): 2018~2026·2fold — `oos_excesses=[-0.1515,-0.0002]`(O(1)·폭발 0)·`r2_measurable=True`·`fold_decays=(None,None)`(IS≤0)·`passed=False`. **🎯 momentum decile 정직 fail**(음수 excess·IS≤0·깨끗한 측정 = 측정 artifact 아닌 진짜 무알파). plan §"깨끗한 측정 후 fail → 다팩터 피벗 정당" 도달.
   - **full canonical(2000~2026·10fold) = ≥20g** 필요(fold9 IS ~24년 peak>14g·8hr) — 공식 validated 판정용. 단 축소 게이트+sanity 가 fail 일관 → 결론 견고. 다음 = 다팩터(ROE/PB) 결합 피벗(범위 밖·후속).
