...
2. ✅✅ **공식 신뢰구간 게이트 완주 — validated=FALSE 정직 판정(2026-06-26·full canonical·12g·10fold·26년)**. `:memory:` fix(커밋 6e4694e·ticker_exchanges 무제한 연결 12.95G→4.13G·이전 "≥20g 필요"=오진 정정·기존 profile.py 분해로 규명) 후 공식 CLI 게이트 기본 12g 완주(oom=false·peak~6G).
   - **판정**: G-4✅(10fold)·G-5✅(폐지 62.9%·n=898)·G-7✅(verify·데이터 정제 확인)·G-8✅·**R-2✅(|worst_oos_excess|=0.243≤10·척도 정상)** / G-1·G-2·G-3·G-6❌ → **종합 FAILED·validated=false**. `oos_excesses` 10폴드 **전부 음수**([-0.027,-0.057,-0.222,-0.243,…]·동일비용 유동성 벤치 언더퍼폼)·`fold_decays` 대부분 None(IS≤0). `s6_gate_result.json`(rule_signature `9b27a413…`).
   - **🏁 최종 결론**: ①방법론 교정(ADR-010) 성공=momentum 26년 **공정 측정 달성**(R-2 PASS·폭발 0·구 e80/e7 소멸) ②momentum 단일팩터 decile=**진짜 무알파**(측정 artifact 아님·전 폴드 음수 초과) ③**validated=false=정직 과학 판정**. plan §4.1 "깨끗한 측정 후 fail → 다팩터(ROE/PB) 피벗 정당" **도달**.
   - **▶ 다음 마일스톤 = 다팩터 룰**(momentum + 재무 ROE/PB 결합·범위 밖·후속). ⚠️ **harness drift**: `post-work-check.sh` 가 호스트 python pytest(deps 미설치·false-positive) → docker 로 수정 필요. orphan 진단 컨테이너·`*_out.txt`=사용자 정리(`docker rm` guard 차단).
