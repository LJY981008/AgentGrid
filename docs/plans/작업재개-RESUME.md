...
2. **신뢰구간 게이트 1회(미실행·메모리 해결됨·8hr 시간만)**: `canonical_gate_config()`(decile·2000~2026)·walk_forward. ✅ **메모리 근본 fix 완료(커밋 6e4694e)**: 진짜 hog 는 `_scan.load_ticker_exchanges` 의 `:memory:` 무제한 연결(98M행 GROUP BY 12.95G) — `memory_limit` 캡으로 **12.95G→4.13G**. (이전 "단일 run 9-12GB·≥20g 필요" 결론은 **오진**·RSS 총량만 본 탓·기존 profile.py tracemalloc/rss/duckdb 분해로 규명.) **reduced 게이트 12g 완주 실증**(oom=false·peak 4.85G·7G 헤드룸). → **full canonical 도 기본 12g 안전**(between-run reset 으로 per-run peak ~4-5G·누적 0). pre-registration: 게이트 1회.
   - 실행(기본 12g·호스트 freeze 위험 없음·app·web 정지 권장): `docker run -d --memory 12g --memory-swap 12g -u 999:999 -v $PWD/src:/app/src:ro -v stockpick_parquet-data:/app/data -e STOCKPICK_DATA_DIR=/app/data/parquet -e MALLOC_ARENA_MAX=2 stockpick-app python -m stockpick.backtest.s6_gate` (verify=G-7·전구간 10fold·`s6_gate_result.json`·~8hr). ⚠️ 볼륨 마운트 = `:/app/data`(중첩 주의·`/app/data/parquet` 아님). `docker rm/kill` guard 차단(orphan 컨테이너·`*_out.txt`=사용자 정리).
   - ✅ **축소 게이트 실증 일관**(gate4 14g·gck12 12g 동일): 2018~2026·2fold — `oos_excesses=[-0.1515,-0.0002]`(O(1)·폭발 0)·`r2_measurable=True`·`fold_decays=(None,None)`(IS≤0)·`passed=False`. **🎯 momentum decile 정직 fail**(음수 excess·IS≤0·깨끗한 측정=측정 artifact 아닌 진짜 무알파). plan §"깨끗한 측정 후 fail → 다팩터 피벗 정당" 도달.
   - **공식 validated 판정 = full canonical(2000~2026·10fold·12g·8hr)** — 축소+sanity 가 fail 일관이라 결론(momentum 단일팩터 무알파→다팩터 ROE/PB 피벗) 견고. ⚠️ **harness drift**: `post-work-check.sh` 가 호스트 python pytest(deps 미설치·false-positive) → docker 로 수정 필요.
