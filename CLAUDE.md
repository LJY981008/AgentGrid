...
- API(M3): **FastAPI**+**uvicorn[standard]**(`src/stockpick/api/`) — 수집·랭킹·학습을 HTTP 노출. pydantic 응답계약 = 프론트 단일 출처. CORS=localhost:5173(컨테이너 내부 web). ⚠️ ranking/backtest `meta.validated`·키 비노출 — 이제 하드코딩 아니라 **S6-b 게이트 flip**(`backtest.s6_gate.load_s6_gate_verdict`·`s6_gate_result.json` 의 passed+rule_signature 일치만 true·부재/불일치/실패 false 보수). **현재 false**(2026-06-23 전구간 게이트 판정: **G-7 무결성 fail** — 가격<=0 542,588행·OHLC위반 88,064행 — **99.6% 비-`_old` 일반티커**(원인 미상·`_old` 플레이스홀더 아님), 데이터 신뢰 불가로 momentum OOS 강건성 미평가·단락). validated=true 전제 = 데이터 정제(비-_old 0-price 원인 규명)→게이트 재실행(백테스트 단계 12g OOM·mem>12g 선결)→momentum G-1~G-6 통과(ADR-009·§4.1 미검증 경고 상시)
- 웹앱(M3·M4): PWA (`webapp/`) — **Vite8/React19/react-router7/TS**, 6 nav 화면(랭킹=Dashboard·**추적=Tracking(M4 월 라운드 운용기록·성과)**·데이터·유니버스·학습·백테스트)+404
- 모듈 경계: `data`(수집·저장) / `rules`(Top20 랭킹) / `backtest`(검증 — M2 엔진 구현·골격, S6 게이트 후 신뢰) / **`tracking`(M4 운용 원장·TWR 성과 — data 만 의존 peer)** → `api`/`webapp`(상위 — 하위 조합) — 하위는 상위 import 금지
...
| `migrations/` | alembic PG 마이그레이션(S5-a·ADR-006) — `env.py`(DATABASE_URL→psycopg3)·`versions/`(0001~0003 시장데이터·**0004 추적 4테이블**[portfolio_round·trade·cash_flow·corporate_action — M4·사용자 데이터라 PG 1차 진실 예외]). compose app 에 마운트. 직접 DDL 금지 |
| `src/stockpick/` | 도메인 계약(`types.py` = 기획 §6, **FinancialFact** 포함) + `data/`(수집·저장·`db.py` PG repo·Parquet→PG 단방향 동기·`export_stock_snapshot`(stock→JSON 스냅샷 S5-d)·`universe.py` 종목마스터 S5-b·`bulk.py` 다년 EOD 벌크 S5-c+**후처리 재구조화·`--finalize` 복구**(commit 직후 `build_cache`)·`duckdb_cache.py` **Parquet→`cache.duckdb` 단일 컬럼 파생캐시+momentum 부분 푸시다운(`momentum_endpoints` SQL 끝점·ADR-007)**)·`rules/` 모듈 + `backtest/`(M2 엔진 — `config·calendar·costs·strategy·ports·adapters·fakes·metrics·engine·benchmark·validation·demo`. 리밸·forward-return·폐지청산·IS/OOS·decay. **adapters `MasterUniverse`(생존편향 유니버스·delisted_at+1 경계 S5-d)·`DuckDBPriceSeriesPort`(`MomentumScorePort`·cache.duckdb 라이브 가속)·`_select_universe`·`_select_price_port`(cache→DuckDB·부재 Parquet 폴백)**·engine `momentum_scores` 분기·**`s6_gate.py` S6-b 신뢰성 게이트**(ADR-009 — G-1~G-8 사전동결 임계·`run_s6_gate`·비용민감도·`compute_rule_signature`/`load_s6_gate_verdict` validated flip·`python -m stockpick.backtest.s6_gate` 격리 CLI[verify→전구간 게이트→`s6_gate_result.json`]·**G-7 fail 시 백테스트 단락**)) + **`tracking/`(M4 추적·보정 루프 — `types`(라운드·거래·현금흐름·SPLIT frozen dataclass)·`repo`(RoundRepository Protocol+Pg·JSONB Decimal↔str)·`fakes`(InMemory)·`ledger`(전역 원장·일중 SPLIT→흐름→trade(id순)·불변식 LedgerError)·`performance`(price return 통일·일별 TWR·4계열+선택/실행효과·판정유보 N<20))** + data 추가: `price_read`(raw close 로더 — adj_factor 미합성)·`benchmark`(SPY 격리 서브트리 `benchmark/`)·eodhd `fetch_splits` |
| `src/stockpick/api/` | FastAPI HTTP 층(M3·M4, 상위 모듈) — `models.py`(pydantic 계약)·`deps.py`(DI·**get_conn 요청단위 commit/rollback**·테스트 override)·`ranking_service.py`(랭킹 오케스트레이션 — ranking 라우트↔라운드 스냅샷 공유)·`routes/{health,dataset,ingest,ranking,learning,backtest,tracking}.py`(**tracking=라운드·거래·성과·마감 9EP**). `python -m stockpick.api` 기동 |
| `tests/` | pytest (픽스처·모킹 — 라이브 데이터 의존 금지) |
| `webapp/` | PWA 대시보드 (M3·M4) — Vite8/React19/router7/TS, `src/{api,components,pages}`. 6화면(랭킹·**추적**·데이터·유니버스·학습·백테스트). 읽기위주·투자로직 프론트 중복 금지([webapp-conventions](.claude/rules/webapp-conventions.md)). ⚠️ dist/node_modules 는 compose 익명볼륨 마스킹(레거시 root 소유 잔재 무력화) |
