...
> **⚠️ 서브에이전트 보고 규칙 (2026-06-12 실측 — 전역 Stop 훅이 종료를 ~8회 막아 재호출함)**: 부모에게는 **마지막 메시지만** 전달된다. 작업 완료 후 재호출(종료 촉구)을 받으면 "종료합니다"류 단답 금지 — **매번 최종 보고 전문을 다시 출력**할 것. 핵심 산출물은 파일(메모리/문서)로도 남길 것.
