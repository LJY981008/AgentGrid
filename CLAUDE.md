...
- API(M3): **FastAPI**+**uvicorn[standard]**(`src/stockpick/api/`) — 수집·랭킹·학습을 HTTP 노출. pydantic 응답계약 = 프론트 단일 출처. CORS=localhost:5173(컨테이너 내부 web). ⚠️ ranking/backtest `meta.validated`·키 비노출 — 이제 하드코딩 아니라 **S6-b 게이트 flip**(`backtest.s6_gate.load_s6_gate_verdict`·`s6_gate_result.json` 의 passed+rule_signature 일치만 true·부재/불일치/실패 false 보수). **현재 false**(2026-06-23 전구간 게이트 판정: **G-7 무결성 fail** — 가격<=0 542,588행·OHLC위반 88,064행 = `_old`/폐지 티커 0-price 플레이스홀더, 데이터 신뢰 불가로 momentum OOS 강건성 미평가·단락). validated=true 전제 = 데이터 정제→게이트 재실행(백테스트 단계 12g OOM·mem>12g 선결)→momentum G-1~G-6 통과(ADR-009·§4.1 미검증 경고 상시)
